 ...
 * 自定义水印生成器
 */
class CustomWatermarkGenerator implements WatermarkGenerator<WaterSensor> {
    ...
    public CustomWatermarkGenerator(Duration duration) {
        if (duration == null) {
            throw new NullPointerException("duration为null");
        }
        outOfOrdernessMillis = duration.toMillis();
        System.out.println(outOfOrdernessMillis);
